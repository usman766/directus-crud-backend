...
function deepMap(object, callback, context) {
  ...
  function recurse(value, key) {
    if (Array.isArray(value)) {
      ...
      return Object.fromEntries(Object.entries(value).map(([key2, val]) => [key2, recurse(val, key2)]));
    } else if (key !== void 0) {
...
import { cloneDeep, get as get3, isPlainObject as isPlainObject2, set } from "lodash-es";
...
function injectFunctionResults(payload, filter) {
  ...
  function processFilterLevel(filter2, parentPath) {
    for (const [key, value] of Object.entries(filter2)) {
      ...
      if (key.startsWith("_") === false && isPlainObject2(value)) {
...
function parseFilterEntry([key, value], accountability, context) {
  if (["_or", "_and"].includes(String(key))) {
    ...
    return { [key]: toArray(val).flatMap((value2) => parseDynamicVariable(value2, accountability, context)) };
